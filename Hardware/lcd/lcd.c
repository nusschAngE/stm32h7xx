...
static void nt35510_flush_data(LCD_COLOR *pData, uint32_t size);
static void SRAM_FMC_Init(void);
...
uint8_t lcd_ShowChar(uint16_t x, uint16_t y, uint8_t ch, uint8_t font, LCD_COLOR fr, LCD_COLOR bg)
{
    ...
    if(wSize > 0)
    {
        ...
        nt35510_flush_data(FontGRAM, wSize);
