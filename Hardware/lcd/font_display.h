...
#ifndef _FONT_DISPLAY_FIELD_H
#define _FONT_DISPLAY_FIELD_H
...
static uint16_t Field_FontGRAM_HZK(LCD_COLOR *ram, uint16_t size, uint16_t gbk, uint8_t font, LCD_COLOR fr, LCD_COLOR bg)
{
	...
	ofs = (190 * hVal + lVal) * libSize; //font data offset

    switch (font)
    {
        case FONT_HZK12:
            SPIFlash_Read(FontData, ofs + FontInfo.hzk12Addr, libSize);
            break;
        case FONT_HZK16:
            SPIFlash_Read(FontData, ofs + FontInfo.hzk16Addr, libSize);
            break;
        case FONT_HZK24:
            SPIFlash_Read(FontData, ofs + FontInfo.hzk24Addr, libSize);
            break;
        case FONT_HZK32:
            SPIFlash_Read(FontData, ofs + FontInfo.hzk32Addr, libSize);
            break;
        default : return 0;
    }
