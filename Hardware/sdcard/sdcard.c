...
#if (SDCARD_DMA_MODE)
    HAL_NVIC_SetPriority(SDCardIRQ, 2, 0);
...
void SD_GetBaseInfo(SD_BaseInfo *info)
{
    ...
    info->BlockNbr      = SDCard.LogBlockNbr;
    info->BlockSize     = SDCard.LogBlockSize;
}
...
uint8_t SD_ReadMultipleBlocks(uint8_t *pBuff, uint32_t BlockAddr, uint32_t BlockNbr)
{
    ...
    */
    ...
    printf("sd_ReadBlocks_DMA, errorstate = 0x%08x\r\n", errorstate);
    if(errorstate == HAL_SD_ERROR_NONE)
    {
        while(timeout--)
        {
            ...
            {
                /* check SD Card state */
                errorstate = (uint32_t)SD_GetCardState();
    ...
    */
	...
	printf("sd_ReadBlocks, errorstate = 0x%08x\r\n", errorstate);
    if(errorstate == HAL_SD_ERROR_NONE)
    {
        while(timeout--)
        {
...
uint8_t SD_WriteMultipleBlocks(uint8_t *pData, uint32_t BlockAddr, uint32_t BlockNbr)
{
    ...
    printf("sd_WriteBlocks_DMA, errorstate = 0x%08x\r\n", errorstate);
    if(errorstate == HAL_SD_ERROR_NONE)
    {
        while(timeout--)
        {
	...
	printf("sd_WriteBlocks, errorstate = 0x%08x\r\n", errorstate);
    if(errorstate == HAL_SD_ERROR_NONE)
    {
        while(timeout--)
        {
