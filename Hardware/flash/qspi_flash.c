...
static uint8_t OPRBUFFER[QSPIFLASH_SECTORSIZE];
...
uint8_t QSPI_Flash_Init(void)
{
    ...
    if(W25Qxx_GetDeviceID() != 0xEF18)
    {
        printf("Flash ID is not W25Q256!\r\n");
        return (1);
    }
...
uint8_t QSPI_Flash_ReadWithCycle(cycle cycleFunc, uint32_t Address, uint16_t rSize)
{
    ...
    if(cycleFunc == NULL) {return (W25Q_ERR_VALUE);}
...
uint8_t QSPI_Flash_WriteNoChk(uint8_t *pBuff, uint32_t Address, uint32_t wSize)
{
    ...
    if((Address + wSize) > QSPIFLASH_MEMSIZE)
    {
        return (QSPIFLASH_ERROR_PARAM);
    }
    /* init value 
    */
    // remian bytes in current page
    remainPageSize = QSPIFLASH_PAGESIZE - Address % QSPIFLASH_PAGESIZE;
    // current write size
    ...
    while(wSize)
    {
        ...
        if(ret != W25Q_OK)
        {
            return (QSPIFLASH_ERROR_WRITE);
        }

        pBuff += curWriteSize;
        Address += curWriteSize;
        wSize -= curWriteSize;
        if(wSize > QSPIFLASH_PAGESIZE)
            curWriteSize = QSPIFLASH_PAGESIZE;
        else
            curWriteSize = wSize;
    }
...
uint8_t QSPI_Flash_WriteWithChk(uint8_t *pBuff, uint32_t Address, uint32_t wSize)
{
    ...
    curSector = Address / QSPIFLASH_SECTORSIZE;
    // offset in current sector
    SectorOfs = Address % QSPIFLASH_SECTORSIZE;
    // remain bytes in current sector
    remainSectorSize = QSPIFLASH_SECTORSIZE - SectorOfs;
	// current write size
    ...
    while(wSize)
    {
        ...
        // read current sector
        ret = W25Qxx_Read(OPRBUFFER, curSector * QSPIFLASH_SECTORSIZE, QSPIFLASH_SECTORSIZE);
        if(ret != W25Q_OK)
        {
            return (QSPIFLASH_ERROR_READ);
        }
        ...
        if(NeedErase)
        {
            ...
            ret = QSPI_Flash_WriteNoChk(OPRBUFFER, curSector * QSPIFLASH_SECTORSIZE, QSPIFLASH_SECTORSIZE);
            if(ret != QSPIFLASH_ERROR_NONE)
                return (QSPIFLASH_ERROR_WRITE);
        }
        else
        {
            ret = QSPI_Flash_WriteNoChk(pBuff, Address, curWriteSize);
            if(ret != QSPIFLASH_ERROR_NONE)
            {
                return (QSPIFLASH_ERROR_WRITE);
            }
        ...
        wSize -= curWriteSize;
        /* new write size */
        if(wSize > QSPIFLASH_SECTORSIZE)
            curWriteSize = QSPIFLASH_SECTORSIZE;
        else
            curWriteSize = wSize;
    }
...
static void QSPI_Init(void)
{
    ...
    QSPI_Handler.Init.FlashSize = (QSPIFLASH_MEMSIZE - 1);//32Mbyte
...
static uint8_t W25Qxx_EraseSector(uint32_t sector)
{
    uint32_t addr = sector * QSPIFLASH_SECTORSIZE;
    uint8_t ret = 0;
