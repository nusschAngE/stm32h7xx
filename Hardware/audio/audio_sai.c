...
*/
const SampleRateConf_Typedef SAI_SRConf[]=
{
...
#define SAI_SRCFG_TALSIZE   (sizeof(SAI_SRConf)/sizeof(SampleRateConf_Typedef))
...
uint8_t Audio_SAITxConfig(SAI_TxInitTypedef *Init)
{	
    uint8_t idx = 0, ret = 0;
    ...
    {
        if((Init->frequency / 10) == SAI_SRConf[idx].SampleRate)  break;
    }
    /* not find */
    if(idx == SAI_SRCFG_TALSIZE) {return (AUDIO_SAIERROR_PARAM);}
	...
	if(ret != HAL_OK) {return (AUDIO_SAIERROR_CLK);}
	...
	if(ret != HAL_OK) {return (AUDIO_SAIERROR_INIT);}
	...
	if(ret != HAL_OK) {return (AUDIO_SAIERROR_DMAINIT);}
    /*  */
	AudioSAI_EnableDMA();
	/* DMA buffer */
    ...
    if(AudioBuffer0 && AudioBuffer1)
    {
    	ret = HAL_DMAEx_MultiBufferStart(&AudioDMA_Handle, (uint32_t)AudioBuffer0, (uint32_t)&(AUDIO_SAI->DR), 
   		...
   		if(ret != HAL_OK) {return (AUDIO_SAIERROR_DMASTART);}
        IsMultiBuffer = TRUE;
    }
    else
    {
...
void DMA1_Stream5_IRQHandler(void)
{
	...
	if(__HAL_DMA_GET_FLAG(&AudioDMA_Handle, DMA_FLAG_TCIF1_5) != RESET)
	{
		...
		if(audioStateCode == AUDIO_STATE_STREAM_END) {return ;}
		if(CodecForceStop == TRUE) {return ;}
