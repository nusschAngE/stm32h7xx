...
static void sdram_ControllerInit(void)
{
...
/* SDRAM IC Initialize */
static uint8_t sdram_HardwareInit(void)
{
    uint32_t temp = 0;
...
void SDRAM_RWTestFunc(void)
{
	...
	pbuff = (volatile uint16_t*)(SDRAM_ADDRESS);
	for(i = 0; i < TSIZE; i++)
	{
		*pbuff = Tbuff[i];
		pbuff++;
	...
	{	
  		...
  		if(temp != Tbuff[i])
  		{
  			printf("error, i = %d, temp = %d, val = %d\r\n", i, temp, Tbuff[i]);
  			Count++;
  		}
  		pbuff++;
 	}	
	printf("SDRAM Test finish, error = %d\r\n", Count);
}
#endif
