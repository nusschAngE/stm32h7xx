...
/*  STATIC 
*/
static uint8_t wav_GetFmtContent(WavFmt_Typedef *fmt, uint8_t *fmtBuff, uint32_t size);
static uint8_t wav_GetExpandContent(WavExpand_Typedef *expand, uint8_t *expBuff, uint32_t size);
static uint8_t wav_GetFactContent(WavFact_Typedef *fact, uint8_t *factBuff, uint32_t size);
...
*/
uint8_t Wav_GetHeader(WavHeader_Typedef *header, uint8_t *pData, uint32_t pSize, uint32_t *bDecode)
{
    uint8_t ret = 0;
    uint8_t ChunkIdx = 0;
    uint8_t cidx = 0;
...
*/
uint8_t Wav_AudioDataProc(WavHeader_Typedef *header, uint8_t *pBuff, uint32_t buffSize, const uint8_t *pData, uint32_t dataSize, uint32_t *bFill)
{
	uint32_t i = 0;
