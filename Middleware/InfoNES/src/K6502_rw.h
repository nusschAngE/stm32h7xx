 ...
 */
  ...
  switch ( wAddr & 0xe000 )
  {
	...
	byRet = NES->PPU_R7;
        ...
        // Read PPU Memory
        NES->PPU_R7 = PPUBANK[ addr >> 10 ][ addr & 0x3ff ];
      ...
      if ( ( wAddr & 0x7 ) == 0x2 )   /* PPU Status */
      {
        ...
        if ( NES->PPU_Scanline >= SCAN_VBLANK_START && !( NES->PPU_R0 & R0_NMI_VB ) )
        {
          NES->PPU_R0 &= ~R0_NAME_ADDR;
  ...
  switch ( wAddr & 0xe000 )
  {
    ...
    case 0x2000:  /* PPU */
      switch ( wAddr & 0x7 )
      {
        case 0:    /* 0x2000 */
          NES->PPU_R0 = byData;
          NES->PPU_Increment = ( NES->PPU_R0 & R0_INC_ADDR ) ? 32 : 1;
          NES->PPU_NameTableBank = NAME_TABLE0 + ( NES->PPU_R0 & R0_NAME_ADDR );
          NES->PPU_BG_Base = ( NES->PPU_R0 & R0_BG_ADDR ) ? ChrBuf + 256 * 64 : ChrBuf;
          NES->PPU_SP_Base = ( NES->PPU_R0 & R0_SP_ADDR ) ? ChrBuf + 256 * 64 : ChrBuf;
	    ...
	    NES->PPU_Temp = ( NES->PPU_Temp & 0x8FFF ) | ( ( ( (WORD)byData ) & 0x07 ) << 12);
          ...
          {
            // H-Scroll Register
            NES->PPU_Scr_H_Next = byData;
            NES->PPU_Scr_H_Byte_Next = NES->PPU_Scr_H_Next >> 3;
        ...
        case 7:   /* 0x2007 */
          {
            ...
            // Write to PPU Memory
            if ( addr < 0x2000 && NES->byVramWriteEnable )
            {
              // Pattern Data
              NES->ChrBufUpdate |= ( 1 << ( addr >> 10 ) );
	    ...
	    if ( addr & 3 )
            {
              // Palette
              PPURAM[ addr ] = byData;
              NES->PalTable[ addr & 0x1f ] = NesPalette[ byData ];
            }
          }
          break;
      }
