...
#define GUI_TOUCH_X_MIN 0
#define GUI_TOUCH_X_MAX 480
#define GUI_TOUCH_Y_MIN 0
#define GUI_TOUCH_Y_MAX 800
...
int  GUI_TOUCH_GetxPhys (void) {
  return tpPoints.xPos[0];
}

int  GUI_TOUCH_GetyPhys (void) {
  return tpPoints.yPos[0];
}
...
void GUI_TOUCH_Exec (void) {
  ...
  if (tpPoints.actPoints) {
    // Touch screen is pressed

    if (TouchOrientation & GUI_SWAP_XY) {
      TouchPhysX = GUI_TOUCH_GetyPhys();
      TouchPhysY = GUI_TOUCH_GetxPhys();
    } else {
      TouchPhysX = GUI_TOUCH_GetxPhys();
      TouchPhysY = GUI_TOUCH_GetyPhys();
    }
